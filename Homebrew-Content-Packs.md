...
To use the extension, make sure you're working in an **EMPTY FOLDER**, create an `lcp_manifest.json` file inside of it, and fill out its required fields. The extension should activate once it detects the manifest file. To run the build task, first, if you're on Windows, make sure you have 7zip installed and [its folder added to your PATH](https://docs.alfresco.com/4.2/tasks/fot-addpath.html) (make sure to restart VSCode after doing this). Mac and Linux should work right away (uses the `zip` command). Press `Ctrl+Shift+P`, and search for `Tasks: Run Build Task`. You should see a `compcon: Build .LCP package` option - select it and your .LCP file should rapidly build itself.

## Working on Mac
If you are building .LCP's on Mac without using VS Code, note that the Mac file browser's zipping option will add an extra layer of folders inside the .ZIP, causing CompCon to not recognize the LCP. To get around this, either [use VS Code](#the-vscode-extension) or create the .ZIP using the command line `zip` tool.
